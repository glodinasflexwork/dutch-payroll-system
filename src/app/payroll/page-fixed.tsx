...
export default function PayrollPage() {
  ...
  const calculatePayroll = async () => {
    if (selectedEmployees.length === 0 || !payPeriodStart || !payPeriodEnd) {
      setError('Please select at least one employee and pay period');
      return;
    }
    ...
    try {
      ...
      for (const employeeId of selectedEmployees) {
        ...
        if (response.ok) {
          ...
          setError(`Error calculating for employee ${employeeId}: ${errorData.error}`);
        }
      }
      ...
      setError(`Failed to calculate payroll: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
      setLoading(false);
    }
  };
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (!isDryRun) {
          ...
          console.log(`Payroll processed successfully for ${selectedEmployees.length} employee(s)!`);
          // Refresh payroll records
          await fetchPayrollRecords();
        } else {
          console.log(`Payroll calculated for ${selectedEmployees.length} employee(s) (dry run - not saved)`);
        }
        
        // Show errors if any
        if (errors.length > 0) {
          const errorMessages = errors.map((err: any) => 
            `${err.employee?.name || 'Unknown'}: ${err.error}`
          ).join(', ');
          setError(`Some employees had errors: ${errorMessages}`);
        }
      ...
      setError(`Failed to process payroll: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
      setProcessing(false);
    }
  };
                ...
                {batchResults.length > 0 ? (
                  <div className="space-y-6">
                    ...
                    <div className="bg-gray-50 rounded-lg p-4">
                      <h4 className="font-medium text-gray-900 mb-3">Batch Processing Results</h4>
